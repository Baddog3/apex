...
- [x] Шаг 1: имя + дата рождения (date picker)
- [x] Шаг 2: город рождения (`CityInput` autocomplete)
- [x] Шаг 3: время рождения (time picker) + кнопка «Не знаю» / Skip
- [x] Прогресс-бар (3 точки)
...
- [x] `POST /api/onboarding/complete` — сохраняет в profiles, вызывает chart/compute
...
- [x] Если `birth_time IS NULL` — баннер на экранах Карта и Сегодня
- [x] Модалка / страница `/profile/birth-time` для дозаполнения
- [x] При добавлении времени → пересчёт карты
...
- [x] API `GET /api/horoscope/today`
- [x] Логика: транзиты → шаблонные ключевые темы → AI генерирует 2–3 предложения
- [x] **Кэш на день:** один вызов AI на пользователя в сутки, результат в БД (`daily_horoscopes`)
- [x] Fallback без AI: статический текст по главному транзиту

**Готово когда:** экран показывает уникальный текст, обновляется раз в сутки.

### Шаг 4.2 — UI экрана

- [x] Приветствие: «Привет, {имя}»
- [x] Карточка гороскопа (крупный текст, минимализм)
- [x] Карточка главного транзита: «Сатурн □ твоя Луна»
- [x] Pull-to-refresh (опционально)
...
**Фаза 5, Шаг 5.1** — компонент `NatalChartWheel` (SVG-колесо натальной карты).

Handoff: [2026-06-11-phase-4-handoff.md](../handoff/2026-06-11-phase-4-handoff.md)
